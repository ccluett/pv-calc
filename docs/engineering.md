...
| `nasa_ring_stiffened_shell_external_pressure` | 6.0.0 | NASA SP-8007 Rev. 2 Eq. 64/65 with Eqs. 82-91 ring stiffnesses and Eq. 91 torsion, fixed 0.75 adjustment, and a mode search over every axial half-wave count whose half-wave spans at least two ring spacings (always `m=1`) and an expanding `n>=2`. Shorter half-waves fall between rings, where the smeared stiffness does not apply and the inter-ring check does; two spacings is this project's screen, since NASA asks only that the smeared theory be checked for sufficiently large spacing. The result reports its boundary assumptions, including freely warping ends (incremental `N_x = 0` at the supports), and the excluded axisymmetric (`n=0`) mode. Its inter-ring bay reuses the smooth-cylinder model, including the Eqs. 30-32 correction when a complete compressive curve is supplied. NASA reads that correction at the circumferential stress of the buckling shell, so the bay's material comparisons, the correction and the proportional-limit screen alike, use the periodic bay's mid-bay hoop membrane stress rather than the unstiffened `p*r/t`; the rings carry part of the hoop load. Without a proportional limit or curve, the bay's elastic pressure still enters the lowest-pressure comparison. When the bay falls in the smooth model's moderate/long correlation overlap, its capacity is withheld on every material path and no lowest pressure is formed; a note says why, and the global and yield pressures are still reported. A mode pressure is labelled elastic only when its stress, the bay's mid-bay hoop stress or the global mode's nominal `p*r/t`, is within a supplied proportional limit; with only a yield strength it is an elastic upper bound above yield and undetermined below it. The orthotropic global modes remain elastic: `global_elastic_applicability` labels an over-limit global pressure as an elastic upper bound but neither corrects nor withholds it. With a yield strength, the axisymmetric solution of a periodic bay (von Sanden and Günther, in the Wilson and PD 5500 form) gives Pc5 and ring yield: the pressures at which the mean hoop stress in the shell at mid-bay, and in the ring, reaches yield in a perfect shell, with the pressure acting at the mid-surface radius as in the PD 5500 form. Exact equilibrium of pressure on the outer surface would raise those mean hoop stresses by `R_o/R = 1 + t/(2R)`, 2.5% at `R/t = 20`, and lower the yield pressures to match; the form is kept because the published PD 5500-style worked example (JMSE 2020) reproduces only with the mid-surface load. Ring first yield is the pressure at which the ring's largest hoop stress, at its smallest radius, reaches yield: the free edge of an internal ring, the base of an external one, where each translating section strains most. They are not collapse pressures and are not part of the lowest buckling pressure. Text and summary output show the lowest buckling pressure and its mode, and the three yield pressures |
...
search, with `maximum_axial_half_waves_m` and the two-spacing screen
`minimum_ring_spacings_per_axial_half_wave` beside it, and the
