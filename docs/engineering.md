...
| `nasa_smooth_cylinder_external_pressure_buckling` | 5.1.0 | NASA SP-8007 Rev. 2 Eqs. 19-29 at shell mid-surface radius; capacity released at every `gamma*Z` except the moderate/long correlation overlap. A complete compressive Ramberg-Osgood curve (`ramberg_osgood_n` with `compressive_proof_stress`) applies the Eqs. 30-32 correction and releases the corrected capacity and margin. Without a curve, a result above the proportional limit remains an elastic upper bound with a null margin (`released_pending_plasticity`). Reference-only data retain the numerical result and margin as `released_unqualified_material`. Requires `R_mid/t > 10`. Reports Roark Table 35 case 20 as a comparator that sets no capacity. The `elastic_applicability` screen labels the applied `p*r/t` comparison and sets no margin. `circumferential_stress_basis` names the stress compared with material data: `p*r/t` here, or a ring-stiffened bay's mid-bay hoop stress when the ring model evaluates its bay |
| `nasa_ring_stiffened_shell_external_pressure` | 6.0.0 | NASA SP-8007 Rev. 2 Eq. 64/65 with Eqs. 82-91 ring stiffnesses and Eq. 91 torsion, fixed 0.75 adjustment, and a mode search over every axial half-wave longer than one ring spacing (always `m=1`) and an expanding `n>=2`. Sampled at the rings, a sine longer than one spacing moves them with the smeared strain energy; at one spacing the rings can sit at the nodes, and shorter half-waves fall between rings, where the smeared stiffness does not apply and the inter-ring check does. This is this project's screen, since NASA asks only that the smeared theory be checked for sufficiently large spacing; when the minimum sits on it and a shorter wave would be lower, `axial_half_wave_limit_binding` is true and a note says so. NASA notes that its orthotropic equations lose accuracy against Love or Sanders theory for `n <= 4` (printed p. 35) and that a more accurate discrete-ring theory gives somewhat lower pressures (p. 38); widely spaced or deep eccentric rings need such a check. The result reports its boundary assumptions, including freely warping ends (incremental `N_x = 0` at the supports), and the excluded axisymmetric (`n=0`) mode. Its inter-ring bay reuses the smooth-cylinder model, including the Eqs. 30-32 correction when a complete compressive curve is supplied. NASA defines that correction at the circumferential stress `p*r/t` of an unstiffened shell; between rings, which carry part of the hoop load, the bay's material comparisons, the correction and the proportional-limit screen alike, extend it to the periodic bay's mid-bay hoop membrane stress on a valid record. When a corrected bay pressure exceeds Pc5 a note says interframe collapse can govern. Without a proportional limit or curve, the bay's elastic pressure still enters the lowest-pressure comparison. When the bay falls in the smooth model's moderate/long correlation overlap, its capacity is withheld on every material path and no lowest pressure is formed; a note says why, and the global and yield pressures are still reported. A mode pressure is labelled elastic only when its stress, the bay's mid-bay hoop stress or the global mode's nominal `p*r/t`, is within a supplied proportional limit; with only a yield strength it is an elastic upper bound above yield and undetermined below it. The orthotropic global modes remain elastic: `global_elastic_applicability` labels an over-limit global pressure as an elastic upper bound but neither corrects nor withholds it. With a yield strength, the axisymmetric solution of a periodic bay (von Sanden and Günther, in the Wilson and PD 5500 form) gives Pc5 and ring yield: the pressures at which the mean hoop stress in the shell at mid-bay, and in the ring, reaches yield in a perfect shell, with the pressure acting at the mid-surface radius as in the PD 5500 form. Exact equilibrium of pressure on the outer surface would raise the unstiffened mean hoop stress by `R_o/R = 1 + t/(2R)`, 2.5% at `R/t = 20`, and the bay's by slightly less, since the axial load's Poisson term changes too; the form is kept because the published PD 5500-style worked example (JMSE 2020) reproduces only with the mid-surface load, and DAPS4 also takes the hoop load at the mean radius, applying `R_o` only to the axial load (Renzi, IHTR 2944, Eqs. 43 and 51). Ring first yield is the pressure at which the ring's largest hoop stress, at its smallest radius, reaches yield: the free edge of an internal ring, the base of an external one, where each translating section strains most. It is hoop stress only, in a perfect shell; out-of-roundness bending of the frame, which peaks at the fibre farthest from the neutral axis, is not included. They are not collapse pressures and are not part of the lowest pressure. The same periodic bay solved with its beam-column term, the Pulos-Salerno solution as Renzi documents it for DAPS4 (IHTR 2944, Eqs. 7 and 43-64), gives `beam_column_bay`: mid-bay and frame stresses on both surfaces at the applied pressure, tension positive, with their plane-stress von Mises values, the membrane stresses, deflections, and the ring hoop stress; it takes the hoop load at the mean radius and the axial load `p R_o^2 / (2R)`, and needs Renzi's `gamma < 1`. With a yield strength, Lunchick's axisymmetric collapse (Renzi Eqs. 65-66) applies the plastic reserve factor phi3 to the pressure at which the outer surface at mid-bay first yields; this collapse pressure enters the lowest pressure beside the buckling modes, as DAPS4 takes the minimum over modes. It is a perfect-shell, elastic-perfectly plastic estimate for identical rings without end bays; DAPS4's comparison with machined aluminium models covers bay parameters theta from 1.0 to 2.5, and a long bay whose mid-bay bending relieves the outer surface is flagged. The production closed form, an independent characteristic-root solution, and DAPS4's published example output agree. Text and summary output show the lowest buckling or collapse pressure and its mode, the three yield pressures, the collapse pressure, and the bay's surface von Mises stresses |
